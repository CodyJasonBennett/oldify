...
function oldify(from, to) {
  ...
  fs.readdirSync(from).forEach(element => {
    ...
    if (fs.lstatSync(path.join(from, element)).isFile()) {
      ...
      const file = filePath.replace(fileName, transpile(fileName))

      fs.writeFileSync(path.join(to, file), transpile(data), 'utf-8');
    } else {
      oldify(path.join(from, element), path.join(to, transpile(element)));
    }
  });
}
